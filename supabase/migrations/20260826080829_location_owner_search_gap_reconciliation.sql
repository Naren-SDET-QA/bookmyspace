-- Reconcile the owner/location functions missing from DEV while preserving
-- the existing location tables, indexes, and RLS policies.
...
revoke all on function public.create_owner_venue_with_location(text, uuid, text, text, text, double precision, double precision, integer, numeric, uuid) from public;
revoke all on function public.update_owner_venue_with_location(uuid, text, uuid, text, text, text, double precision, double precision, integer, numeric, boolean, uuid) from public;
revoke all on function public.get_location_descendants(uuid) from public;
revoke all on function public.create_owner_venue_with_location(text, uuid, text, text, text, double precision, double precision, integer, numeric, uuid) from anon, service_role;
revoke all on function public.update_owner_venue_with_location(uuid, text, uuid, text, text, text, double precision, double precision, integer, numeric, boolean, uuid) from anon, service_role;
revoke all on function public.get_location_descendants(uuid) from service_role;
