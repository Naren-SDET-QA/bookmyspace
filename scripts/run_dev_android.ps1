# Launch Flutter Android against the remote DEV Supabase project.
...
$oneSignalAppId = $values['ONESIGNAL_APP_ID']
$remoteUrl = $false
...
if (-not [string]::IsNullOrWhiteSpace($oneSignalAppId)) {
  $defines += "--dart-define=ONESIGNAL_APP_ID=$oneSignalAppId"
}

Write-Host "Launching BookMySpace DEV on $Device (APP_ENV=development)."
