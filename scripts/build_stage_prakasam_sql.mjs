...
const report = JSON.parse(fs.readFileSync('.tmp/ap_prakasam_function_halls.json', 'utf8'));
...
sql += `  update public.venue_import_jobs
    set status = 'review',
        ...
        completed_at = now()
    where id = v_job;\n`;
sql += "  raise notice 'job=% staged=% dup=%', v_job, v_staged, v_dup;\n";
sql += 'end;\n';
sql += '$body$;\n';
sql += 'select id, status, venues_fetched, venues_staged, venues_duplicates from public.venue_import_jobs order by created_at desc limit 1;\n';
sql += "select count(*)::int as pending_review from public.venue_import_staging s join public.venue_import_jobs j on j.id=s.job_id where j.state='Andhra Pradesh' and s.status='pending_review';\n";
sql += 'commit;\n';

fs.writeFileSync('.tmp/stage_prakasam.sql', sql);
console.log(JSON.stringify({ venues: venues.length, sql: '.tmp/stage_prakasam.sql' }));
